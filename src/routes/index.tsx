...
function OrderSummary() {
  const details = [
    ...
    ["Conteúdo", "8 capítulos · +34 páginas"],
    ["Entrega", "Por e-mail, em minutos"],
    ["Acesso", "Vitalício, sem mensalidade"],
    ["Dispositivos", "Celular, tablet e computador"],
    ["Garantia", "7 dias incondicional"],
  ];
  return (
    <div className="mt-10 overflow-hidden rounded-3xl border border-gold/25 bg-surface/50 backdrop-blur">
      <div className="flex items-center gap-2 border-b border-gold/15 bg-surface-elevated/60 px-5 py-4 text-[0.65rem] uppercase tracking-[0.28em] text-muted-foreground sm:px-8">
      ...
      <div className="grid gap-8 p-5 sm:p-8 lg:grid-cols-[1.1fr_1fr]">
        ...
        <div>
          <div className="grid grid-cols-[64px_minmax(0,1fr)] items-start gap-4">
            <img
              src={ebookMockup}
              alt="Capa do e-book Cosmetologia Regenerativa Aplicada — Método 4A"
              className="w-full rounded-md object-contain"
              loading="lazy"
            />
            <div className="min-w-0">
              <div className="font-display text-lg leading-snug">
                Cosmetologia Regenerativa Aplicada — Método 4A
              </div>
              <div className="mt-1 text-sm text-muted-foreground">Erik dos Santos · 1 unidade</div>
            </div>
          </div>

          <dl className="mt-6 divide-y divide-gold/10 border-t border-gold/10">
            {details.map(([k, v]) => (
              <div key={k} className="grid grid-cols-[minmax(0,0.8fr)_minmax(0,1fr)] gap-4 py-2.5 text-sm">
        ...
        {/* Valores */}
        <div className="rounded-2xl border border-gold/20 bg-background/50 p-5 sm:p-6">
          <div className="space-y-3 text-sm">
            <div className="flex items-center justify-between gap-4">
              <span className="text-muted-foreground">Valor original</span>
              <span className="text-muted-foreground line-through decoration-destructive/60">R$ 247,00</span>
            </div>
            <div className="flex items-center justify-between gap-4">
              <span className="text-muted-foreground">Desconto de lançamento</span>
              <span className="text-gold">− R$ 60,00</span>
            </div>
            <div className="flex items-center justify-between gap-4">
              <span className="text-muted-foreground">Frete</span>
              <span className="text-foreground">Grátis (digital)</span>
            </div>
          </div>

          <div className="mt-5 flex items-baseline justify-between gap-4 border-t border-gold/15 pt-5">
            <span className="text-xs uppercase tracking-[0.25em] text-muted-foreground">Total</span>
            <span className="font-display text-3xl leading-none text-gold-gradient sm:text-4xl">R$ 187,00</span>
          </div>

          <div className="mt-5 space-y-2">
            <div className="flex items-center justify-between gap-3 rounded-xl border border-gold/30 bg-gold/10 px-4 py-3 text-sm">
              <span className="text-foreground">À vista no Pix ou cartão</span>
              <span className="font-medium text-gold">R$ 187,00</span>
            </div>
            <div className="flex items-center justify-between gap-3 rounded-xl border border-border px-4 py-3 text-sm">
              <span className="text-foreground">Parcelado no cartão</span>
              <span className="font-medium text-foreground">10x R$ 22,03</span>
            </div>
          </div>
          ...
          <div className="mt-4 flex items-center justify-center gap-2 text-xs text-muted-foreground">
            <Lock className="h-3.5 w-3.5 shrink-0 text-gold" /> Ambiente seguro · dados criptografados
          </div>
        </div>
      </div>
    </div>
